...
mongoexport --db jesuscout --collection (collection) --out matches.json --pretty    # get the entire Heartland Collection
mongoexport --db jesuscout --collection (collection) --query '{"scoutName": "max"}' --out matches.json --pretty #only get specified query from Heartland Collection

# TABLEAU CONVERSION

mongodrdl --db jesuscout --collection (collection) --out schema.drdl
